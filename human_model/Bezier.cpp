...
void Bezier::draw_solid_rbc(){
	...
	for(i=0;i<loop;i++){
		for(j=0;j<T;j++){
			...
			glTexCoord2f(theta/(2*Pi), t);
			glVertex3f(mesh_points[i][j].x, mesh_points[i][j].y,mesh_points[i][j].z);
			glTexCoord2f(theta/(2*Pi), 1-t);
			glVertex3f(mesh_points[i+1][j].x, mesh_points[i+1][j].y,mesh_points[i+1][j].z);
			glTexCoord2f(theta/(2*Pi), 1-t);
			glVertex3f(mesh_points[i+1][j+1].x, mesh_points[i+1][j+1].y,mesh_points[i+1][j+1].z);
			glTexCoord2f(theta/(2*Pi), t);
	...
	if (R == 360)
	{
		...
		for (j = 0; j< T; j++)
		{
			...
			glNormal3f(mesh_normals[i][j].x, mesh_normals[i][j].y, mesh_normals[i][j].z);
			glBegin (GL_POLYGON);
			glTexCoord2f(0.01*theta/(2*Pi), 0.01);
			glVertex3f(mesh_points[i][j].x, mesh_points[i][j].y,mesh_points[i][j].z);
			glTexCoord2f(0.01*theta/(2*Pi), 0.01);
			glVertex3f(mesh_points[0][j].x, mesh_points[0][j].y,mesh_points[0][j].z);
			glTexCoord2f(0.01*theta/(2*Pi), 0.01);
...
/*
 * generate_bezier
 * 	a method the generates the bezier points
 * 	Parameters:
 * 		bezier_curve_points: the number of points to generate
 * 	Returns:
 * 		none
 *
 */
void Bezier::generate_bezier(GLint bezier_curve_points ){
	Point bezier_curve_point, bezier_curve_tangent;
	GLfloat u;
	GLint *C;
	GLint k;
...
/*
 * set_texture
 * 	a method to set the texture of the body
 * 	Parameters:
 * 		t: the texture to set
 * 	Returns:
 * 		none
 */
void Bezier::set_texture(GLuint t){
	printf("Body Texture :%i", t);
	texture = t;
}
